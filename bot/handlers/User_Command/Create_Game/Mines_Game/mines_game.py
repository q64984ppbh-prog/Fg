...
async def call_start_game_mines(call: CallbackQuery, state: FSMContext):
    ...
    # Берём последнюю ставку
    from handlers.User_Command.Create_Game.start_create_game import get_last_bet
    ...
    await call.answer()
    await process_mines_start(call, state, user_id, message_id, amount, call.from_user.username, call.from_user.first_name, call.bot)

async def process_mines_start(source, state, user_id, message_id, amount, username, first_name, bot):
    if await db.users.try_reserve_balance(user_id, amount):
        count_mines = await db.game_mines.count_mines(user_id)
        value = await logs_create_mines_game(user_id, username, first_name, count_mines, amount, bot)
        game_id = await db.game_mines.create_mines_game(user_id, value, count_mines)
        await db.users.add_turnover(user_id, value)
        await channel_functions.check_user_level_up(user_id, bot)
        photo = FSInputFile('photo/start.jpg')
        await bot.send_photo(chat_id=user_id, photo=photo, caption=clean(f"""<b>🕹 Игра #{game_id} запущена</b>

💣 Мин: <b>{count_mines} шт.</b>
💰 Ставка: <b>{value:.2f}$</b>

ℹ️ Выберите клетку для <b>открытия</b>"""), reply_markup=generate_mines_field(game_id))
    else:
...
async def open_cell_handler(call: CallbackQuery):
    ...
    if exploded:
        ...
        await call.message.edit_reply_markup(reply_markup=generate_mines_field(game_id, opened, mines, count_mines, bet_amount, finish=True, exploded=True))
        await call.answer("💥 Вы попали на мину!", show_alert=True)
        return
    await db.game_mines.open_cell(game_id, opened)
    await call.message.edit_reply_markup(reply_markup=generate_mines_field(game_id, opened, mines, count_mines, bet_amount))
    await call.answer()
...
async def take_prize_handler(call: CallbackQuery):
    ...
    await db.game_mines.finish_game(game_id)
    await call.message.edit_reply_markup(reply_markup=generate_mines_field(game_id, opened, mines, count_mines, bet_amount, finish=True, exploded=False))
