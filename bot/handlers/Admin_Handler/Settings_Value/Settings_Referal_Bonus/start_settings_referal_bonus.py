...
async def call_change_status_referal_bonus(call: CallbackQuery):
    ...
    if flag == 1:
        ...
        await call.answer(caption="🎁 Реферальный конкурс завершен!", show_alert=True)
    elif flag == 0:
        await db.admin.update_value('Flag_Bonus_Ref', 1)
        await call.answer(caption="🎁 Мы запустили реферальный конкурс, который разорит ваш кошелек!", show_alert=True)
